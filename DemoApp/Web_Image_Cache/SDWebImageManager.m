...
- (void)imageCache:(SDImageCache *)imageCache didNotFindImageForKey:(NSString *)key userInfo:(NSDictionary *)info
{
    ...
    if ([delegate isKindOfClass:[UIButton class]]) {
        UIButton *button = (UIButton *)delegate;
        activity = (UIActivityIndicatorView *)[button viewWithTag:100000];
        if (!activity) {
            ...
            [activity setCenter:CGPointMake(button.frame.size.width/2.0f,button.frame.size.height/2.0f)];
    ...
    else if ([delegate isKindOfClass:[UIImageView class]]) {
        UIImageView *imageView = (UIImageView *)delegate;
        activity = (UIActivityIndicatorView *)[imageView viewWithTag:100000];
        if (!activity) {
            activity = [[[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite]autorelease];
            [activity setTag:100000];
            [activity setActivityIndicatorViewStyle:UIActivityIndicatorViewStyleWhite];
            [activity setHidesWhenStopped:YES];
            [activity setCenter:CGPointMake(imageView.frame.size.width/2.0f,imageView.frame.size.height/2.0f)];
